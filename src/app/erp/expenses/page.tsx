  ...
  return (
    <AppShell userName={session.user.name} scope="tenant">
      <div className="space-y-6">
        <SectionHeader title={isAr ? "المصروفات" : "Expenses"} description={isAr ? "سجلات مصروفات الشركة مع ربط الموردين وسجل التدقيق." : "Tenant spending records with supplier links and audit history."} icon={ReceiptText} />
        <MetricGrid
          metrics={[
            { label: isAr ? "المصروفات" : "Expenses", value: aggregate._count.toLocaleString(), icon: ReceiptText, detail: isAr ? "سجلات نشطة" : "Active records" },
            { label: isAr ? "الإنفاق" : "Spend", value: formatMoney(Number(aggregate._sum.amount ?? 0), company.defaultCurrency), icon: ReceiptText, detail: isAr ? "مصروفات مسجلة" : "Recorded expenses" },
            { label: isAr ? "الموردون" : "Suppliers", value: suppliers.length.toLocaleString(), icon: ReceiptText, detail: isAr ? "موردون متاحون" : "Available vendors" },
            { label: isAr ? "العملة" : "Currency", value: company.defaultCurrency, icon: ReceiptText, detail: isAr ? "عملة الشركة الافتراضية" : "Default company currency" }
          ]}
        />
        <Card>
          <CardHeader>
            <CardTitle>{isAr ? "إنشاء مصروف" : "Create expense"}</CardTitle>
          </CardHeader>
          <CardContent>
            <form action={createExpenseAction} className="grid gap-3 md:grid-cols-6">
              ...
              <Input name="category" placeholder={isAr ? "التصنيف" : "Category"} required />
              <Input name="description" placeholder={isAr ? "الوصف" : "Description"} />
        ...
        <DataTable
          headers={isAr ? ["التاريخ", "المورد", "التصنيف", "الوصف", "المبلغ", "الإجراءات"] : ["Date", "Supplier", "Category", "Description", "Amount", "Actions"]}
