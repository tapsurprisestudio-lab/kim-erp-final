...
async function getCompanyMetrics(companyId: string) {
  try {
    const [company, products, customers, invoices, inventory, recentLogs] = await Promise.all([
      prisma.company.findUnique({
        ...
        select: {
          ...
          subscriptions: { select: { plan: { select: { name: true } } }, orderBy: { createdAt: "desc" }, take: 1 }
        }
      }),
    ...
    const locale = normalizeLocale(company?.defaultLanguage ?? "en");
    const currency = company?.defaultCurrency ?? "USD";
    return {
      company,
      locale,
      kpis: [
        { label: t(locale, "products"), value: products.toLocaleString(), delta: "+0%", icon: Package },
        { label: t(locale, "customers"), value: customers.toLocaleString(), delta: "+0%", icon: Users },
        { label: t(locale, "sales"), value: formatMoney(Number(invoices._sum.total ?? 0), currency), delta: "+0%", icon: TrendingUp },
        { label: t(locale, "inventory"), value: Number(inventory._sum.quantity ?? 0).toLocaleString(), delta: "+0%", icon: FileText }
      ],
      users: customers,
    ...
    console.error("[dashboard:tenant-load-failed]", { companyId, error });
    const locale = normalizeLocale("en");
    return {
      company: null,
      locale,
      kpis: [
        { label: t(locale, "products"), value: "0", delta: "+0%", icon: Package },
        { label: t(locale, "customers"), value: "0", delta: "+0%", icon: Users },
        { label: t(locale, "sales"), value: formatMoney(0, "USD"), delta: "+0%", icon: TrendingUp },
        { label: t(locale, "inventory"), value: "0", delta: "+0%", icon: FileText }
      ],
      users: 0,
      recentLogs: []
    };
...
export default async function DashboardPage({
      ...
      <AppShell userName={session.user.name} scope="tenant">
        <Card className="mx-auto max-w-xl">
          <CardContent className="p-6">
            <h1 className="text-xl font-bold text-slate-950">Account setup issue</h1>
            <p className="mt-2 text-sm text-slate-600">Your account is not linked to a company. Contact support.</p>
          </CardContent>
        </Card>
      </AppShell>
    );
  ...
  const metrics = await getCompanyMetrics(session.user.companyId);
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            <h1 className="text-2xl font-bold tracking-normal text-slate-950">
              {t(metrics.locale, "dashboard")}
            </h1>
            <p className="mt-1 text-sm text-slate-500">
              {`${metrics.company?.name ?? session.user.companyName ?? "Company"} - ${t(metrics.locale, "companyWorkspace")}`}
            </p>
          </div>
          <Badge variant="success">
            <ShieldCheck className="mr-1 size-3" />
            {t(metrics.locale, "tenantProtected")}
          </Badge>
        </div>
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {[
            ...
            [t(metrics.locale, "language"), `${metrics.company?.defaultLanguage?.toUpperCase() ?? "EN"} / ${metrics.company?.defaultCurrency ?? "USD"}`]
          ].map(([label, value]) => (
            <Card key={label}>
        ...
        <section className="grid gap-4 xl:grid-cols-[1.2fr_0.8fr]">
          <Card>
            <CardHeader>
              <CardTitle>{t(metrics.locale, "revenueOverview")}</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{t(metrics.locale, "recentActivity")}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              {metrics.recentLogs.length === 0 ? (
                <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">
                  {t(metrics.locale, "noActivity")}
                </p>
              ) : (
